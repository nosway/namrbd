...
var reloadPolicies = map[string]map[string]FieldPolicy{
	ProcessGateway: {
		...
		"gateway.sbs_authenticated_admin_endpoint": {ReloadRestart, "the authenticated admin client is already connected"},
	...
	ProcessSBSService: {
		...
		"sbs_service.summary":       {ReloadRestart, "the status and metrics read path is selected when the service starts"},
