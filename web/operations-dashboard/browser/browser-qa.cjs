...
(async () => {
  ...
  try {
    for (const fixture of ['ok', 'degraded', 'stale']) {
      ...
      await page.locator('[data-dashboard-root] .app-header').waitFor();
      ...
      if (!text.includes('READ ONLY')) throw new Error(`${fixture}: read-only badge not rendered`);
      const mutationCount = await page.locator('button, input[type=submit]').evaluateAll(nodes => nodes.filter(node => !/refresh/i.test(node.textContent || '')).length);
