...
| Legacy RawKV Payload Store (not for production) | `namrbd-gateway` (`store-backend=tikv`)          | RawKV        | compressed volume binary payload segments                 |
...
Configure the physical deployment mapping in `topology.yaml` prior to
provisioning. The following complete example uses three dedicated PD hosts
(`pd1`, `pd2`, `pd3`) and three dedicated TiKV hosts (`tikv1`, `tikv2`,
`tikv3`). The management host must resolve each of these names through DNS or
`/etc/hosts`, and the same names must be reachable from every PD and TiKV
host.

```yaml
# topology.yaml -- dedicated NAMRBD metadata cluster: 3 PD + 3 TiKV
global:
  user: "root"
  ssh_port: 22
...
This is a PD+TiKV-only cluster for NAMRBD metadata, so it intentionally omits
`tidb_servers`. Change the deployment account and paths if `root` and the shown directories are not appropriate for the target environment. If you want non-root installation, please refer to [https://docs.pingcap.com/tidb/stable/tiup-cluster-no-sudo-mode/](https://docs.pingcap.com/tidb/stable/tiup-cluster-no-sudo-mode/).

### 4.3 Execute Deploy & Start Runbooks

```bash
tiup cluster deploy namrbd-db v8.5.8 ./topology.yaml --user root -p
...
```bash
sbs-service --pd-endpoints="pd1:2379,pd2:2379,pd3:2379" --keyspace="namrbd-prod"
```
