...
| `sbs-service` summary projection and operations views | Transactionally derived, 64-way sharded cluster counters; bounded rebuild/outbox state; signed fleet observations; and read-only `namrbd.sbs.observability.v1` aggregate, page, point, membership, capacity, reclaim, operation, MCP, GUI, and workflow views. | Authoritative `sbs-service` records, `sbs-data` health/capacity detail, and gateway/control-plane membership/liveness summaries. | Raw TiKV replacement, storage mutation authority, silent full-scan fallback, live iSCSI HA authority, GUI/MCP action authority, or a replacement for AdminService and gateway control-plane state. |
...
| SBS cluster high availability and metadata upkeep | `sbs-service` stores cluster bootstrap, leader/admin operations, node membership, node health, volume metadata, placement, allocation pages, and transition state in TiKV. It commits derived summary deltas or outbox entries with the source mutation, and keeps maintenance work-ready indexes as rebuildable access paths. `sbs-data` stores node-local payload and local execution state in Pebble. | `sbsctl` and gateway-facing control paths call `sbs-service` admin/published-view APIs. Normal fleet polling reads bounded summary shards; detail callers use revision-pinned pages or point APIs. Maintenance workers claim indexed, leased work instead of rediscovering the whole cluster. | Raw TiKV source records remain authoritative. A derived projection or index may be rebuilt, but must never create storage truth, silently trigger an all-scan, or move cluster authority into a gateway or client. |
| Backup/DR control plane | Backup targets, policies, run records, restore-drilled artifact availability, retention holds, purge dry-run guardrails, Backup/DR status summaries, remote DR replication links, recovery points, and shipping manifests. | Enterprise `sbsctl backup`, `sbsctl dr link`, `sbsctl dr recovery-point`, and `sbsctl dr shipping-manifest` call `sbs-service` admin APIs. Fixture validation tools may emit JSON evidence, but product state is persisted by `sbs-service`. | Backup/DR automation is enterprise-only. Product records cover DR link, recovery-point, manifest, and shipping-worker admission state; remote transfer completion, standby import, promote/demote, and failover support remain gated. |
...
A derived index or projection is an access path, not a second authority.
Summary deltas share the authoritative source transaction or enter a durable
outbox, maintenance work indexes are verified against the source record before
a lease is granted, and rebuilds are cursor-bounded and revision-pinned. If a
projection is stale, partial, or requires rebuild, that state is returned to the
caller instead of being hidden by an unbounded compatibility scan.
