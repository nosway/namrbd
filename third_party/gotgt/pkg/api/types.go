...
type TargetPortGroup struct {
	GroupID               uint16
	AsymmetricAccessState uint8
	Preferred             bool
	ImplicitALUASupported bool
	ExplicitALUASupported bool
...
type ZeroCapableBackingStore interface {
	Zero(offset, tl int64) error
}
...
type RemoteZeroCapableBackingStore interface {
	Zero(int64, int64) (int, error)
}
