...
func (s *ISCSITargetDriver) NewTarget(tgtName string, configInfo *config.Config) error {
	...
	for tpgt, portalIDArrary := range targetConfig.TPGTs {
		...
		scsiTPG := ensureSCSITargetPortGroup(&tgt.SCSITarget, uint16(tpgtNumber))
		if alua, ok := targetConfig.TPGTALUA[tpgt]; ok {
			scsiTPG.AsymmetricAccessState = alua.AccessState & 0x0f
			scsiTPG.Preferred = alua.Preferred
			scsiTPG.ImplicitALUASupported = alua.ImplicitSupported
			scsiTPG.ExplicitALUASupported = alua.ExplicitSupported
		}
	...
	stgt.TargetPortGroups = tgt.SCSITarget.TargetPortGroups
	return nil
}

func ensureSCSITargetPortGroup(target *api.SCSITarget, groupID uint16) *api.TargetPortGroup {
	for _, tpg := range target.TargetPortGroups {
		if tpg.GroupID == groupID {
			return tpg
		}
	}
	tpg := &api.TargetPortGroup{
		GroupID:               groupID,
	...
	if events&DATAIN != 0 {
		...
		go func() {
			...
			if conn.state == CONN_STATE_CLOSE {
				log.Debugf("iscsi connection[%d] closed", conn.cid)
				conn.close()
				IPMutex.Lock()
		...
		case IOSTATE_RX_BHS:
			...
			if err != nil {
				...
				case errors.Is(err, io.EOF):
					log.Debugf("initiator closed connection while waiting for BHS: %v", err)
				case isClosedNetworkConnectionError(err):
					log.Debugf("initiator connection closed while waiting for BHS: %v", err)
				case errors.Is(err, io.ErrUnexpectedEOF):
					log.Warnf("partial BHS read before connection close: %v", err)
				default:
...
func (s *ISCSITargetDriver) iscsiExecLogin(conn *iscsiConnection) error {
	...
	if conn.loginParam.iniCSG == SecurityNegotiation {
		negoKeys, err := conn.processSecurityData()
		if err != nil {
			return err
		}
		conn.loginParam.respKV = negoKeys
	...
	if !conn.loginParam.paramInit {
		if err := s.BindISCSISession(conn); err != nil {
			...
			conn.loginParam.statusDetail = loginStatusDetailTargetNotFound
...
SendRemainingData:
	...
	if conn.req != nil && conn.req.OpCode == OpLogoutReq {
		conn.state = CONN_STATE_CLOSE
		return
	}
	switch conn.state {
