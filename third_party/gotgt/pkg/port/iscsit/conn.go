...
func (conn *iscsiConnection) buildRespPackage(oc OpCode, task *iscsiTask) error {
	...
	case OpLoginResp:
		...
		conn.resp.ISID = conn.loginParam.isid
		conn.resp.TSIH = conn.loginParam.tsih
		if conn.session != nil {
			conn.resp.TSIH = conn.session.TSIH
		}
		conn.resp.ExpCmdSN = conn.req.CmdSN
		conn.resp.MaxCmdSN = conn.req.CmdSN
		conn.resp.StatusClass = conn.loginParam.statusClass
		conn.resp.StatusDetail = conn.loginParam.statusDetail
		negoKeys := conn.loginParam.respKV
		if conn.req.CSG != SecurityNegotiation && conn.loginParam.statusClass == loginStatusSuccess {
