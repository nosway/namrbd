...
func bsPerformCommand(bs api.BackingStore, cmd *api.SCSICommand) (err error, key byte, asc SCSISubError) {
	...
	backendSense := func(err error) bool {
		type gotgtSenseError interface {
			GotgtSense() (byte, SCSISubError, bool)
		}
		if err == nil {
			return false
		}
		var senseErr gotgtSenseError
		if !errors.As(err, &senseErr) {
			return false
		}
		senseKey, senseASC, ok := senseErr.GotgtSense()
		if !ok {
			return false
		}
		key = senseKey
	...
	case api.WRITE_SAME, api.WRITE_SAME_16:
		err, key, asc = performWriteSame(bs, cmd)
		if err != nil {
			goto sense
		}
		break
...
func performWriteSame(bs api.BackingStore, cmd *api.SCSICommand) (error, byte, SCSISubError) {
	if cmd.TL == 0 {
		return nil, HARDWARE_ERROR, ASC_INTERNAL_TGT_FAILURE
	}
	pattern, err := writeSamePattern(cmd)
	if err != nil {
		return err, ILLEGAL_REQUEST, ASC_INVALID_FIELD_IN_CDB
	}
	if !allZeroBytes(pattern) {
		return fmt.Errorf("WRITE SAME only supports zero-pattern payloads"), ILLEGAL_REQUEST, ASC_INVALID_FIELD_IN_CDB
	}
	if cmd.SCB[1]&0x08 != 0 {
		...
		if err != nil {
			return err, MEDIUM_ERROR, ASC_WRITE_ERROR
		}
		return nil, HARDWARE_ERROR, ASC_INTERNAL_TGT_FAILURE
	}
	zeroer, ok := bs.(api.ZeroCapableBackingStore)
	if !ok {
		return fmt.Errorf("WRITE SAME zero is not supported by backing store"), ILLEGAL_REQUEST, ASC_INVALID_FIELD_IN_CDB
	}
	if err = zeroer.Zero(int64(cmd.Offset), int64(cmd.TL)); err != nil {
		return err, MEDIUM_ERROR, ASC_WRITE_ERROR
	}
	return nil, HARDWARE_ERROR, ASC_INTERNAL_TGT_FAILURE
}

func writeSamePattern(cmd *api.SCSICommand) ([]byte, error) {
	if cmd.OutSDBBuffer == nil {
		return nil, fmt.Errorf("WRITE SAME requires data-out buffer")
	}
	if cmd.Device == nil || cmd.Device.BlockShift >= 63 {
		return nil, fmt.Errorf("WRITE SAME invalid logical block size")
	}
	blockBytes64 := uint64(1) << cmd.Device.BlockShift
	if blockBytes64 == 0 || blockBytes64 > uint64(int(^uint(0)>>1)) {
		return nil, fmt.Errorf("WRITE SAME logical block size exceeds process limit")
	}
	available := int(cmd.OutSDBBuffer.Length)
	if available == 0 || available > len(cmd.OutSDBBuffer.Buffer) {
		available = len(cmd.OutSDBBuffer.Buffer)
	}
	blockBytes := int(blockBytes64)
	if available < blockBytes {
		return nil, fmt.Errorf("WRITE SAME data-out buffer shorter than one logical block")
	}
	return cmd.OutSDBBuffer.Buffer[:blockBytes], nil
}
