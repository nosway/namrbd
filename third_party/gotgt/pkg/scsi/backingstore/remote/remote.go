...
func (bs *RemBackingStore) Unmap(bd []api.UnmapBlockDescriptor) (err error) {
	if bs.RemBs == nil {
		return fmt.Errorf("remote backing store is not initialized")
	}
	for _, desc := range bd {
		if desc.TL == 0 {
			continue
...
func (bs *RemBackingStore) Zero(offset, length int64) error {
	if bs.RemBs == nil {
		return fmt.Errorf("remote backing store is not initialized")
	}
	zeroer, ok := bs.RemBs.(api.RemoteZeroCapableBackingStore)
	if !ok {
		return fmt.Errorf("remote backing store does not support zero")
	}
	_, err := zeroer.Zero(offset, length)
	return err
}
