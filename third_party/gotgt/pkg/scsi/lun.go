...
func NewSCSILu(bs *config.BackendStorage) (*api.SCSILu, error) {
	...
	if value := strings.TrimSpace(bs.SCSIVendorID); value != "" {
		lu.Attrs.VendorID = value
	}
	if value := strings.TrimSpace(bs.SCSIProductID); value != "" {
		lu.Attrs.ProductID = value
	}
	if value := strings.TrimSpace(bs.SCSIProductRev); value != "" {
		lu.Attrs.ProductRev = value
	}
	if value := strings.TrimSpace(bs.SCSIID); value != "" {
		lu.Attrs.SCSIID = value
	}
	if value := strings.TrimSpace(bs.SCSISerial); value != "" {
		lu.Attrs.SCSISN = value
	}
