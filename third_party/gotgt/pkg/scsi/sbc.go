...
func NewSBCDevice(deviceType api.SCSIDeviceType) api.SCSIDeviceProtocol {
	...
	sbc.SCSIDeviceOps[api.MAINT_PROTOCOL_IN] = NewSCSIDeviceOperation(SPCServiceAction, []*SCSIServiceAction{
		{ServiceAction: 0x0A, CommandPerformFunc: SPCReportTargetPortGroups},
...
func SBCReadWrite(host int, cmd *api.SCSICommand) api.SAMStat {
	...
	case api.WRITE_SAME, api.WRITE_SAME_16:
		...
		// NAMRBD only maps explicit zero-pattern WRITE SAME. LBDATA/PBDATA
		// variants need command-specific fixtures before being advertised.
		if scb[1]&0x06 != 0 {
			key = ILLEGAL_REQUEST
			asc = ASC_INVALID_FIELD_IN_CDB
			goto sense
	...
	if err != nil {
		...
		BuildSenseData(cmd, key, asc)
		if key == ILLEGAL_REQUEST || key == DATA_PROTECT {
			return api.SAMStatCheckCondition
		}
