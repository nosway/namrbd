...
func InquiryPage0x80(host int, cmd *api.SCSICommand) (*bytes.Buffer, uint16) {
	...
	serial := strings.TrimSpace(cmd.Device.Attrs.SCSISN)
	if serial == "" {
		serial = fmt.Sprintf("gotgt-%v", cmd.Device.UUID)
	}
	copy(scsisn, []byte(fmt.Sprintf("%-36s", serial)))
	buf.Write(scsisn)
	return buf, pageLength
}
...
func InquiryPage0x83(host int, cmd *api.SCSICommand) (*bytes.Buffer, uint16) {
	...
	if targetPort != nil && portID == 0 {
		portID = targetPort.RelativeTargetPortID
		portGroup = FindTargetGroup(cmd.Target, portID)
	}
	targetPortName := ""
	if targetPort != nil {
		targetPortName = strings.TrimSpace(targetPort.TargetPortName)
	}
	if targetPortName == "" {
		targetPortName = fmt.Sprintf("%s,t,0x%02x", cmd.Target.Name, portID)
	}
	...
	portName = util.StringToByte(targetPortName, 4, 256)
...
func SPCReportTargetPortGroups(host int, cmd *api.SCSICommand) api.SAMStat {
	if cmd.Target == nil || cmd.InSDBBuffer == nil || len(cmd.SCB) < 10 {
		return spcInvalidFieldInCDB(cmd)
	}
	allocationLength := util.GetUnalignedUint32(cmd.SCB[6:10])
	...
	descriptors := &bytes.Buffer{}
	for _, tpg := range cmd.Target.TargetPortGroups {
		if tpg == nil || len(tpg.TargetPortGroup) == 0 {
			continue
		}
		state := tpg.AsymmetricAccessState & 0x0f
		first := state
		if tpg.Preferred {
			first |= 0x80
		}
		descriptors.WriteByte(first)
		descriptors.WriteByte(0x0f)
		binary.Write(descriptors, binary.BigEndian, tpg.GroupID)
		descriptors.WriteByte(0x00)
		descriptors.WriteByte(0x00)
		binary.Write(descriptors, binary.BigEndian, uint16(len(tpg.TargetPortGroup)))
		for _, port := range tpg.TargetPortGroup {
			descriptors.WriteByte(0x00)
			descriptors.WriteByte(0x00)
			binary.Write(descriptors, binary.BigEndian, port.RelativeTargetPortID)
		}
	}

	data := &bytes.Buffer{}
	data.Write(util.MarshalUint32(uint32(descriptors.Len())))
	data.Write(descriptors.Bytes())
	payload := data.Bytes()
	limit := int(allocationLength)
	if limit > len(payload) {
		limit = len(payload)
	}
	if limit > len(cmd.InSDBBuffer.Buffer) {
		limit = len(cmd.InSDBBuffer.Buffer)
	}
	copy(cmd.InSDBBuffer.Buffer, payload[:limit])
	if allocationLength > uint32(limit) {
		cmd.InSDBBuffer.Resid = allocationLength - uint32(limit)
	} else {
		cmd.InSDBBuffer.Resid = 0
	}
	return api.SAMStatGood
...
func spcInvalidFieldInCDB(cmd *api.SCSICommand) api.SAMStat {
	if cmd.InSDBBuffer != nil {
		cmd.InSDBBuffer.Resid = 0
	}
	BuildSenseData(cmd, ILLEGAL_REQUEST, ASC_INVALID_FIELD_IN_CDB)
