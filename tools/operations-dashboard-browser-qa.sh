...
OUT_DIR="${NAMRBD_BROWSER_QA_OUT_DIR:-$ROOT_DIR/.cache/operations-dashboard-browser-qa}"
PORT="${NAMRBD_BROWSER_QA_PORT:-18080}"
PLAYWRIGHT_IMAGE="${PLAYWRIGHT_IMAGE:-mcr.microsoft.com/playwright:v1.55.0-noble}"
RUNTIME_PROFILE="${NAMRBD_BROWSER_QA_RUNTIME_PROFILE:-}"
...
UPDATE_BASELINE="${NAMRBD_BROWSER_QA_UPDATE_BASELINE:-0}"
...
if [[ "$ready" != true ]]; then
	printf '{"result":"fail","entrypoint":"operations-dashboard-browser-qa","error_count":1,"first_error":"dashboard HTTP server did not become ready","last_error":"dashboard HTTP server did not become ready"}\n'
	exit 1
fi
...
if [[ ! -s "$BASELINE" ]]; then
	printf '{"result":"fail","entrypoint":"operations-dashboard-browser-qa","error_count":1,"first_error":"screenshot checksum baseline missing","last_error":"screenshot checksum baseline missing"}\n'; exit 1
fi
if ! diff -u "$BASELINE" "$OUT_DIR/actual.sha256" >"$OUT_DIR/screenshot-diff.txt"; then
	printf '{"result":"fail","entrypoint":"operations-dashboard-browser-qa","browser_render_executed":true,"screenshot_count":3,"screenshot_checksum_verified":false,"error_count":1,"first_error":"screenshot checksum baseline mismatch","last_error":"screenshot checksum baseline mismatch"}\n'; exit 1
fi
jq --arg runtime_profile "$RUNTIME_PROFILE" --arg baseline_path "web/operations-dashboard/browser/screenshots.$RUNTIME_PROFILE.sha256" '. + {result:"ok",entrypoint:"operations-dashboard-browser-qa",browser_render_executed:true,screenshot_count:3,screenshot_checksum_verified:true,runtime_profile:$runtime_profile,baseline_path:$baseline_path,error_count:0,first_error:"",last_error:""}' "$OUT_DIR/browser-evidence.json" | tee "$OUT_DIR/summary.json"
